...
の2つを紐づけて対応させることで、整数値で簡単に全探索を行えるようにしていきます。
...
となっており、これを2進数で復元するとき**選ぶときを1、選ばないときを0**とすると2進数で  $110_{(2)}$ と表すことができます。つまり、

- **$i$ 番目のものを選ぶなら、2進数の右から $i$ 桁目を1にする**
- **$i$ 番目のものを選ばないなら、2進数の右から $i$ 桁目を0にする**

だけで「いくつか選ぶ方法」を2進数で表すことができるのです。

この話を具体的に図で表すと以下のようになります。

{{< figure src="image2.png" >}}

見てわかる通り、選んだものに対応して2進数の桁が1になっています。
...
よって、この問題は整数の選び方を表した整数値 $0,1,2,...,2^M-1$ をbit全探索することで時間に余裕を持って求めることができます。実装については
...
※動的計画法(DP)をご存知の方であればこの問題もDPで解けることが分かると思いますが、今回は整数1つ1つの値が非常に大きいので計算量が $O(NV)$ となってしまい実行時間に間に合いません。(また、配列の要素数が最大で $10^9$ を超えるのでMLE(メモリ制限超過)やRE(実行時エラー)が発生します。)
...
例題2の実装では、数字の「いくつか選ぶ方法」の配列Sを作成していましたが、実際の問題では配列Sを作成することはほとんどありません。その代わりに
...
よって、この問題は集合の選び方を表した整数値 $0,1,2,...,2^M-1$ をbit全探索することで時間に余裕を持って求めることができることが分かります。
...
ことで条件の判定を行うことができます！( `set` についての詳細はAPG4bの[AA - 3.03.STLのコンテナ](https://atcoder.jp/contests/apg4b/tasks/APG4b_aa)内の細かい話欄を見てください。)
...
> 【問題】<br>
$H_1$ 行 $W_1$ 列の行列 $A$ と、 $H_2$ 行 $W_2$ 列の行列 $B$ が与えられます。<br>
> - $1 \leq i \leq H_1$ かつ $1 \leq j \leq W_1$ を満たす整数の組 $(1, j)$ について、行列 $A$ の $i$ 行目 $j$ 列目の要素は $A_{i, j}$ です。 
> - $1 \leq i \leq H_2$ かつ $1 \leq j \leq W_2$ を満たす整数の組 $(1, j)$ について、行列 $B$ の $i$ 行目 $j$ 列目の要素は $B_{i, j}$ です。
> 
...
##### 解法
さて、勘の鋭い方であれば既に気づいたと思いますが、bit全探索を使える場面は
 - **非常に小さい制約が与えられたとき**

であることが分かります。その上で、肝心なのは

> 「どこを決めれば全体が決まるか」という視点を忘れずに考察することは、制約が大きくても重要<br>
> [bit 全探索 - けんちょんの競プロ精進記録](https://drken1215.hatenablog.com/entry/2019/12/14/171657) より

ということをよく考えることです。
今回の問題では、 $H_1, H_2, W_1, W_2$ が全て小さい制約となっています。果たして、この制約を全探索することによって、全ての状態を全探索できるのかを考えていきます。
...
であることと同じであることが分かります。すなわち、 **1つずつ行と列を削除しても、同時に複数の行と列を削除しても最終的な状態は変化しないのです。**
例えば、入力例1で
-  2列目を削除 → 3行目を削除 → 1行目を削除 → 4列目を削除
-  1行目、3行目、2列目、5列目を同時に削除

とそれぞれ $A$ に操作を行うと以下のように状態が変化します。

**2列目を削除 → 3行目を削除 → 1行目を削除 → 4列目を削除した場合**

{{< figure src="image4.png" >}}

**1行目、3行目、2列目、5列目を同時に削除した場合**
...
を行うにはどうすればよかったのでしょうか。$H_1$ 個あるものをいくつか選ぶ方法は $2^{H_1}$ 通り、$W_1$ 個あるものをいくつか選ぶ方法は $2^{W_1}$ 通りあるので、そうです。bit全探索を使って全ての方法である $2^{(H_1+W_1)}$ 通りを全列挙することができます。この組合せは最大でも $2^{(10+10)} = 1048576 \approx 10^6$ 通りなので全探索可能な範囲です。

今回は行の選び方を表した整数値 $0,1,2,...,2^{H_1}-1$ をbit全探索するループと列の選び方を表した整数値と $0,1,2,...,2^{W_1}-1$ をbit全探索するループの2重ループをすることで全探索することができます。

実装ですが少し量が多いので一つ一つ落ち着いてコードを書いていきましょう。大まかなステップとしては
- $A$ のどの行を選んで残すのかをbit全探索 + $A$ のどの列を選んで残すのかをbit全探索する(2重ループ)
- 行と列のbitを復元して、実際に操作を行った行列 $A'$ を構築する
- $A'$ と $B$ が同じかどうかを判定する

の3ステップです。

まず、残す行のbit全探索と残す列のbit全探索を2重ループを書くと以下のようになります。
```cpp
// Aのどの行を残すかをbit全探索
...
この判定方法としては、`nh.size()` と $H_2$ , `nw.size()` と $W_2$ が等しいか判定する方法か、bitの1の個数を返す `__builtin_popcount(bit)` を利用する方法の2つあります。
...
vector<int> nh, nw;
for(int i = 0; i < H1; i++){
    if((bit1 >> i) & 1){ // もしi番目の行を選んだなら
        nh.push_back(i); // nhに番号を入れておく
    }
}
for(int j = 0; j < W1; j++){
    if((bit2 >> j) & 1){ // もしj番目の列を選んだなら
        nw.push_back(j); // nwに番号を入れておく
    }
}
