...
の選択肢が存在します。<br>
つまり、数字を **「選ぶ」「選ばない」** の2通りの選択肢が3つあります。よって、**3個の整数の中からいくつか選ぶ方法は $2^3 = 8$ 通り**存在し、それを列挙すれば良いことになります。
...
例題1では「3個の整数」から「選ぶ」「選ばない」の **2通りある選択肢の選び方** の全列挙をforループで行いましたが、実はbit全探索の場合でも同じ方針で全探索をすることができます。<br>
一般に、bit全探索は

- 複数あるものから **「選ぶ」「選ばない」** などの **2通りある選択肢の選び方がある場合**、あり得る選び方を全列挙する[^1]

[^1]: $N$ 個の要素を持つ集合 $\\{0, 1, 2, \dots, N-1\\}$ の部分集合を全列挙することと同じ意味です。

ことができます！また、**$N$ 個のものから「いくつか選ぶ方法」は $2^N$ 通り** あるので、言い換えると

 - **$2^N$ 通り存在する $N$ 個のものから「いくつか選ぶ方法」を全列挙する**
...
長くなりましたが、例題2をbit全探索で実装すると以下の通りになります！ 
