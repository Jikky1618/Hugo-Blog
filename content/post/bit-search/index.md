...
競技プログラミングでアルゴリズムを初めて学ぶ人にとって最初の壁となるのがbit全探索だと思います。僕自身も初めて学んだアルゴリズムがbit全探索であり、学ぶ際に色々苦労しました。
...
よって、表に書いた通り3個の整数の中からいくつか選ぶ方法は $2^3 = 8$ 通り存在します。一般に **$N$ 個のものからいくつか選ぶ方法は $2^N$ 通り存在する**ことが知られています。
...
ここで使えるのがbit全探索です。早速、例題2を解くbit全探索を実装していきましょう。

## bit全探索の実装

bit全探索の基本的な実装方針は以下の通りです。
...
- コード内の `1 << N` は1をN桁左シフトした値 = $2^N$ のことです。
...
「いくつか選ぶ方法」というのは $2^N$ 通りありますが、これは指数的に増加します。すなわち $N$ の値が大きくなると「いくつか選ぶ方法」の数は爆発的に増加します。具体的な処理にもよりますが、**bit全探索が行えるのは $N \leq 20$ くらいの小さい値が限界です。**<br>
計算量だと $O(N 2^N)$ となります。

## 実践問題

それではここで1問、bit全探索で解いてみましょう。
...
$N$ 個の整数の中からいくつかの整数を選ぶ方法は $2^N$ 通りあるので、bit全探索が使えます。また、制約を見ると、 $N \leq 16$ なので多くても $2^{16} = 65536$ 通りしかないです。よって、bit全探索でも時間に余裕を持って求めることができます。
